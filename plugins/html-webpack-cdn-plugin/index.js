  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap('WebpackCdnPlugin', (compilation) => {
      WebpackCdnPlugin._getHtmlHook(compilation, 'beforeAssetTagGeneration', 'htmlWebpackPluginBeforeHtmlGeneration').tapAsync(
        'WebpackCdnPlugin',
        (data, callback) => {
          const cdnModules = Array.isArray(data.plugin.options.cdnModules) ? data.plugin.options.cdnModules : []
          const IS_PROD = ['production', 'prod'].includes(process.env.NODE_ENV)
          if (data.plugin.options.vconsoleFlag && !IS_PROD) {
            if (!cdnModules.includes('vconsole')) {
              cdnModules.push('vconsole')
            }
          }
          if (cdnModules.length > 0) {
            let modules = cdnModules.reduce((prev, next) => {
              prev = prev.concat(this.modules[next])
              return prev
            }, [])
            if (modules) {
              if (this.optimize) {
